// == Beyo WAVE v7.1 — Bass + Pulse + Auto-FPS =====================
window.WaveformVisualizer = (() => {
  ...
  function draw(ts) {
    ...
    // --- Kick detection (low frequencies) ---
    ...
    // --- Middle frequencies for melodic motion ---
    ...
    const centerPulse = 1 + kickLevel * 1.5; // central bars stronger on kick
    ...
    for (let i = 0; i < BAR_COUNT; i++) {
      ...
      // --- amplitude influenced by mid frequencies and wave ---
      const freqIndex = Math.floor(t * (dataArray.length - 1));
      const audioAmp = dataArray[freqIndex] / 255;

      const centerBoost = 1 + (1 - t) * kickLevel;
      const sideBoost = 0.6 + midAvg * (1 - t); // melodic side bars

      const wave = (Math.sin(offsets[i]) + 1) / 2;
      const amplitude = shape * wave * audioAmp * h * HEIGHT_SCALE * pulse * centerPulse * sideBoost;
